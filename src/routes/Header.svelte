...
<header class={`flex ${navIsOpen ? "flex-col text-center" : undefined} lg:flex-row gap-4 p-4 bg-cyan-900 text-cyan-100 sticky top-0`}>
	<h1 class={`${navIsOpen ? "text-center" : "mr-auto"} text-4xl`}>Markus rantanikunen</h1>	

	<ul class={`${!navIsOpen ? "hidden" : "flex flex-col"} lg:flex lg:flex-row gap-8 text-xl`}>
		{#each navItems as navItem}
		<li transition:slide class={`hover:text-cyan-400 transition duration-200 ${navItem.active ? "text-cyan-200" : undefined}`}><a class="block" on:click={toggleNav} href={navItem.href}>{navItem.label}</a></li>
		{/each}
	</ul>
